...
$limit = 100;
foreach ($schema['objects'] as $objName => $params) {
    ...
    $phpClass->setQualifiedName('\\Zilliqa\\DataType\\' . $objName)
        ->setParentClassName('ZilliqaDataType')
        ->setDescription(array(
            'Zilliqa data type ' . $objName .'.',
            '',
    ...
    $destinationPath = TARGET_PATH . '/' . ucfirst($objName) . '.php';
    echo "generated file: $destinationPath \n";
    if ($shouldWriteToDisc) {
        file_put_contents($destinationPath , $fileHeader . $phpClassText);
    }
    else {
...
function makeTypeArrayBody(array $input): string
{
    ...
    foreach ($input['params'] as $name => $type) {
        if (is_array($type)) {
            $data[] = "\t'" . $name . "' => '[" . ZilliqaDataType::getTypeClass($type[0]) . "]',";
        } else {
            $data[] = "\t'" . $name . "' => '" . ZilliqaDataType::getTypeClass($type) . "',";
        }
    }
    $data[] = "];";
    return implode("\n", $data);
...
function makeToArrayBody(array $input): string
{
    ...
    foreach ($input['params'] as $name => $type) {
        if (is_array($type)) {
            $return .= sprintf("\t" . '\'%s\' => \Zilliqa\Zilliqa::valueArray($this->%s, \'%s\'),', $name, $name, ZilliqaDataType::getTypeClass($type[0])) . "\n";
        } elseif (! ZilliqaData::typeMap($type)) {
            $return .= sprintf("\t" . '\'%s\' => $this->%s->toArray(),', $name, $name) . "\n";
        } else {
            $return .= sprintf("\t" . '\'%s\' => $this->%s->val(),', $name, $name) . "\n";
        }
    }
